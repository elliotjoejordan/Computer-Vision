...
def main():
    ...
    # image pre processing for sobel edge detection
    '''
    for j in range(len(imgs_data)):
        current = cv2.Sobel(imgs_data[j].img,cv2.CV_64F,1,0,ksize=5)
        abs_current = np.absolute(current)
        sobel8U = np.uint8(abs_current)
        imgs_data[j].img = sobel8U
    '''
    ...
    svm.save(params.HOG_SVM_PATH + "UppedParameters")
